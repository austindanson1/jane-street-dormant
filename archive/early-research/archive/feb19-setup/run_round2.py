"""
Jane Street Dormant LLM — Round 2 Runner
Priority 1: Reproducibility test (same prompt twice → same activations?)
Priority 2: Complete layer scan for Models 2 and 3
Priority 3: Activation fingerprinting with candidate triggers
"""
import asyncio
import os
import json
...
async def test_reproducibility():
    ...
    for model in MODELS[:1]:  # Model 1 only to conserve credits
        log(f"\n--- {model} ---")
        try:
            run1 = await get_activations(prompt, test_modules, model=model)
        ...
        try:
            run2 = await get_activations(prompt, test_modules, model=model)
        ...
        for mod in test_modules:
            if mod in run1 and mod in run2:
                a, b = run1[mod].flatten(), run2[mod].flatten()
...
async def complete_layer_scan():
    ...
    for model in MODELS:
        ...
        for i in range(0, len(all_mods), CHUNK):
            ...
            try:
                acts = await get_activations(prompt, chunk, model=model)
                all_scan_data[model].update(acts)
            except Exception as e:
                log(f"  Error: {e}")
                if "Negative project balance" in str(e):
...
async def main():
    log(f"Jane Street Dormant LLM — Round 2")
